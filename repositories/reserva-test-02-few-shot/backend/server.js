...
app.post('/api/reservations', (req, res) => {
  ...
  db.run(sql, [tutorId, petId, hostId, startDate, endDate], function(err) {
    ...
    console.log(`[NOTIFICAÇÃO HOST] Nova solicitação ID: ${this.lastID}`);
...
app.post('/api/reservations/:id/cancel', (req, res) => {
  ...
  db.get('SELECT * FROM reservations WHERE id = ?', [reservationId], (err, row) => {
    ...
    // Lacuna: a fonte não informa se concluída pode ser cancelada.
    // Decisão mínima: Bloquear se concluída conforme instrução de escopo.
    if (row.status === 'completed') {
      return res.status(400).json({ error: 'Não é possível cancelar uma reserva concluída' });
    }

    db.run('UPDATE reservations SET status = ? WHERE id = ?', ['cancelled', reservationId], (err2) => {
      if (err2) return res.status(500).json({ error: err2.message });
      console.log(`[NOTIFICAÇÃO HOST] Cancelamento ID: ${reservationId}`);
